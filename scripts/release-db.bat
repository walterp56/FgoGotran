...
echo release-db options:
echo   Set FGOGOTRAN_S3_URI and FGOGOTRAN_CLOUDFRONT_DISTRIBUTION_ID to publish and invalidate automatically.
echo.
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0release-db.ps1" %PUBLISH_ARGS% %*
set "EXIT_CODE=%ERRORLEVEL%"
echo.
if not "%EXIT_CODE%"=="0" (
  echo release-db failed with exit code %EXIT_CODE%.
) else (
  echo release-db finished successfully.
)
echo.
pause
exit /b %EXIT_CODE%
