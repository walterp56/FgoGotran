...
7. 在总览直接复制真实 Endpoint 与 API Key，再和 Model ID 一起填入手机上的 FgoGotran；需要检查时才显示完整值。
...
- 推理速度和请求状态
- 启动、重新启动和停止按钮
- 默认遮盖的 Endpoint；原复制图标会复制真实地址，不会复制遮盖文本
- Model ID
- 默认遮盖的 API Key；原复制图标会复制真实 Key，不会复制遮盖文本
- 原有的复制图标、显示完整 Endpoint、显示完整 API Key 和更换 Key 控制
...
`环境诊断` 只显示检查状态，不显示本机路径、网络地址或硬件信息。运行日志中的 IP、本机路径、设备信息和 API Key 会被永久移除，没有恢复显示入口。日志仍可筛选级别、刷新、复制或清除显示，也不会故意保存翻译 Prompt 或游戏对白。手机连接所需地址只能在总览中由用户主动复制或显示；复制时页面仍可保持遮盖。
...
手机应使用总览中 Endpoint 右侧的复制图标取得真实翻译地址；需要检查时也可点击`显示 Endpoint`，例如：
...
路由器地址不是电脑地址。不要猜测 IP，以总览复制或显示的当前地址为准。
...
1. 选择 `自定义 / 本地 AI`。
2. 点击 Endpoint 右侧的复制图标，将复制到的完整地址填入 API 地址。
3. 填入完全相同的 Model ID。
4. 点击 API Key 右侧的复制图标，并把完整 Key 粘贴到 Android。
5. 保存并运行连接测试。

Endpoint 和 API Key 即使仍显示为遮盖字符，原复制图标取得的也是完整真实值；`显示`按钮只用于人工检查。
